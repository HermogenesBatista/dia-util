...
class DiaUtil:
    ...
    def __init__(self, data_inicio=None, connection_type="sql"):
        if not data_inicio:
            data_inicio = datetime.date.today()
        self.data_inicio = data_inicio
        self.initialize(connection_type)

    def initialize(self, connection_type):
        if connection_type not in ["sql", "django"]:
            raise Exception("Unknow connection type")

        if connection_type == "sql":
            self.connection = ConnSqlite()
        elif connection_type == "django":
            self.connection = Holiday
    ...
    def verifica_feriado(self, date):
        self.feriados = self.connection.select_sql([self.data_inicio, date])
        ...
        while self.results > 0:
            for _ in self.feriados:
                ...
                self.feriados = self.connection.select_sql([temp, date])
    ...
    def conta_dia_util(self, data_inicio, data_fim, ans=False):
        ...
        if (dias_corridos_rest >= 5 or (
                dias_corridos_rest > 2 and (temp_dia_inicio == 5
        ...
        dados.sort()
        self.feriados = self.connection.select_sql(dados)
